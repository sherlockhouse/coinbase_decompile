...
public final class Http2 implements Variant {
    ...
    static final class FrameLogger {
        ...
        static String formatHeader(boolean inbound, int streamId, int length, byte type, byte flags) {
            String formattedType = type < TYPES.length ? TYPES[type] : Util.format("0x%02x", Byte.valueOf(type));
        ...
        static {
            ...
            for (i = 0; i < BINARY.length; i++) {
                BINARY[i] = Util.format("%8s", Integer.toBinaryString(i)).replace(' ', NumericKeypadConnector.ZERO);
            }
            FLAGS[0] = "";
            FLAGS[1] = "END_STREAM";
    ...
    static final class Reader implements FrameReader {
        ...
        public void readConnectionPreface() throws IOException {
            if (!this.client) {
                ...
                if (Http2.logger.isLoggable(Level.FINE)) {
                    Http2.logger.fine(Util.format("<< CONNECTION %s", connectionPreface.hex()));
                }
