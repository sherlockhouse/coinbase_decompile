...
public final class LocaleManager {
    ...
    private static String doGetTLD(Map<String, String> map, Context context) {
        String tld;
        String countryOverride = PreferenceManager.getDefaultSharedPreferences(context).getString(PreferencesActivity.KEY_SEARCH_COUNTRY, null);
        if (!(countryOverride == null || countryOverride.length() <= 0 || "-".equals(countryOverride))) {
            tld = (String) map.get(countryOverride);
            if (tld != null) {
                return tld;
            }
        }
        tld = (String) map.get(getSystemCountry());
