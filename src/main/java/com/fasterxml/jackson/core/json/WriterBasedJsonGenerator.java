...
public final class WriterBasedJsonGenerator extends JsonGeneratorImpl {
    ...
    private void _prependOrWriteCharacterEscape(char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        if (escCode >= 0) {
            ...
            if (ch > 'ÿ') {
                ...
                ptr++;
                buf[ptr] = NumericKeypadConnector.ZERO;
                ptr++;
                buf[ptr] = NumericKeypadConnector.ZERO;
            }
            ptr++;
        ...
        if (escCode >= 0) {
            ...
            if (ptr <= 5 || ptr >= end) {
                ...
                if (ch > 'ÿ') {
                    ...
                    i = ptr + 1;
                    buffer[ptr] = NumericKeypadConnector.ZERO;
                    ptr = i + 1;
                    buffer[i] = NumericKeypadConnector.ZERO;
                }
                i = ptr + 1;
    ...
    private void _appendCharacterEscape(char ch, int escCode) throws IOException, JsonGenerationException {
        if (escCode >= 0) {
            ...
            if (ch > 'ÿ') {
                ...
                buf[ptr] = NumericKeypadConnector.ZERO;
                ptr = i2 + 1;
                buf[i2] = NumericKeypadConnector.ZERO;
            }
            i2 = ptr + 1;
            buf[ptr] = HEX_CHARS[ch >> 4];
            ptr = i2 + 1;
    ...
    private char[] _allocateEntityBuffer() {
        ...
        buf[2] = '\\';
        buf[3] = 'u';
        buf[4] = NumericKeypadConnector.ZERO;
        buf[5] = NumericKeypadConnector.ZERO;
        buf[8] = '\\';
        buf[9] = 'u';
